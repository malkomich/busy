...
INSERT INTO service(service_type_id, hour_schedule_id, role_id) VALUES((SELECT id FROM service_type WHERE name='Tipo 1'), (SELECT id FROM hour_schedule WHERE start_time='22:00:00'), (SELECT id FROM role WHERE branch_id=(SELECT id FROM branch WHERE phone='902202122')));

INSERT INTO booking(person_id, service_id) VALUES((SELECT id FROM person WHERE email='user1@domain.com'), (SELECT id FROM service WHERE hour_schedule_id=(SELECT id FROM hour_schedule WHERE start_time='22:00:00')));